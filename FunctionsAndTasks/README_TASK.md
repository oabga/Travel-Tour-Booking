...
- **Nhập danh sách Hành khách** — Ghi nhận thông tin từng hành khách: họ tên, ngày sinh, SĐT, **CCCD/Hộ chiếu** (`PassengerIdNumber` — bắt buộc với người lớn, để trống với trẻ em), phân loại `PassengerType` (Adult/Child)
...
- **Đăng ký / Đăng nhập** — JWT Authentication với `PasswordHash` (BCrypt) lưu trong bảng `Accounts`, phân quyền qua bảng `Roles` và `AccountRoles`. Thông tin cá nhân khách hàng lưu riêng trong `CustomerProfiles`
...
- **Thống kê cá nhân** — Đếm số booking trong năm (`fn_UserBookingCount`)
...
**13 bảng** (tách Auth thành 4 bảng riêng biệt):
...
> **Lưu ý về hệ thống Auth và phân biệt với Employees:**
> - `Accounts` — tài khoản đăng nhập (Email + PasswordHash). Phân quyền qua bảng trung gian `AccountRoles` → `Roles` (Admin / Staff / Customer).
> - `CustomerProfiles` — thông tin cá nhân (FullName, Phone, DOB, Address) chỉ tồn tại với Account có Role = Customer.
> - `BookingDetails.PassengerIdNumber` — lưu CCCD (12 số) hoặc số hộ chiếu của từng hành khách. Bắt buộc với người lớn (`Adult`), để `NULL` với trẻ em (`Child`).
> - `Employees` — hồ sơ hướng dẫn viên, **không đăng nhập hệ thống**. Admin phân công vào `TourSchedules.EmployeeId`.
> - **Admin là người tạo/sửa/xóa Tour.** Staff chỉ xem.
...
| `Accounts` | Tài khoản đăng nhập (Email + PasswordHash) | 4 |
| `Roles` | Danh sách role: Admin / Staff / Customer | 2 |
| `AccountRoles` | Bảng trung gian gán Role cho Account | 2 |
| `CustomerProfiles` | Thông tin cá nhân khách hàng (FullName, Phone, DOB, Address) | 5 |
| `Bookings` | Đơn đặt tour | 9 |
| `BookingDetails` | Danh sách hành khách (PassengerType + PassengerIdNumber) | 7 |
...
| `vw_BookingDetails` | Chi tiết đơn hàng — JOIN Bookings + Accounts + CustomerProfiles + TourSchedules + Tours + Destinations |
...
| `fn_UserBookingCount` | AccountId, Year | INT | Đếm số booking của một Account trong năm |
...
GET    /api/bookings/account/{id}         Lịch sử booking của tài khoản
```

### Auth & Accounts
```
POST   /api/auth/register                 Đăng ký tài khoản (tạo Account + CustomerProfile)
POST   /api/auth/login                    Đăng nhập → JWT token
GET    /api/accounts/{id}/profile         Xem hồ sơ cá nhân (CustomerProfile)
PUT    /api/accounts/{id}/profile         Cập nhật hồ sơ cá nhân
```
...
database/02_CreateTables.sql        ← Tạo 13 bảng + constraints (BookingDetails có PassengerIdNumber)
database/03_Views.sql               ← Tạo 3 Views
database/04_Functions.sql           ← Tạo 3 Functions (fn_CalcBookingTotal, fn_UserBookingCount, fn_GenerateInvoiceCode)
...
| **TV3** | `feature/auth-customer` | Khách hàng & Bảo mật | Đăng ký/Đăng nhập JWT, RBAC `[Authorize]`, hồ sơ cá nhân, Reviews, `fn_UserBookingCount` |
...
- [ ] ≥ 3 Functions (`fn_CalcBookingTotal`, `fn_UserBookingCount`, `fn_GenerateInvoiceCode`)
...
- [ ] Seed data đủ để demo (Tours, Schedules, Accounts, CustomerProfiles, Bookings, Reviews)
